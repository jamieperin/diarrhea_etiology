...
# 2023 07 26
# Exclude Jafari (Iran) and Siqueira et al (Brazil)
table(grepl("Jafari",inp.int$bibliography))
inp.int<- inp.int[!grepl("Jafari",inp.int$bibliography),]
table(grepl("Siqueira",inp.int$bibliography))
inp.int<- inp.int[!grepl("Siqueira",inp.int$bibliography),]


length(unique(inp.int$bibliography))

#nc<- read.dta13("../data/U5_mcee_covariates_2020320_dy.dta")
#list.files("../../COD_2022/Under five 2022")
nc<- read.dta13("../data/vavr_national_covariates_2023 02 14.dta")
summary(nc$year)
summary(nc$rota)
names(nc)
table(nc$iso3)
nc$whoreg6[which(nc$iso3=="HRV")]
# for Yugoslavia, using Croatia's covariates
yug<- nc[which(nc$iso3=="HRV"),]
yug$iso3<- "YUG"
nc<- rbind(nc, yug)
...
inp.int$rota<- NULL
inp.int<- merge(inp.int, nc[,c("iso3","year", "rota","u5mr",
                               "gni","water_sm",
                               "whoreg6")], by=c("iso3", "year") , all.x=T)
...
inp.int[which(is.na(inp.int$water_sm)),]
...
inp.int<- inp.int[which(inp.int$High.income==0),]
...
summary(inp.int$rota)
summary(nc$rota)
hist(inp.int$rota, main="Rota vac coverage", xlab="Coverage")
...
#inp.int$int<-1
#inp.int$int.std<-1
table(inp.int$whoreg6, useNA="always")
inp.int$whoreg6<- ifelse(inp.int$iso3=="YUG","4_Eur", as.character(inp.int$whoreg6))
table(inp.int$whoreg6)
inp.int$I_Africa<- ifelse(inp.int$whoreg6=="1_Afr",1,0)
inp.int$I_Amr<- ifelse(inp.int$whoreg6=="2_Amr",1,0)
inp.int$I_Sear<- ifelse(inp.int$whoreg6=="3_Sear",1,0)
inp.int$I_Wpr<- ifelse(inp.int$whoreg6=="6_Wpr",1,0)
inp.int$I_Emr<- ifelse(inp.int$whoreg6=="5_Emr",1,0)


nc$I_Africa<- ifelse(nc$whoreg6=="1_Afr",1,0)
nc$I_Amr<- ifelse(nc$whoreg6=="2_Amr",1,0)
nc$I_Sear<- ifelse(nc$whoreg6=="3_Sear",1,0)
nc$I_Eur<- ifelse(nc$whoreg6=="4_Eur",1,0)
nc$I_Wpr<- ifelse(nc$whoreg6=="6_Wpr",1,0)
nc$I_Emr<- ifelse(nc$whoreg6=="5_Emr",1,0)

covs<-c('rota','u5mr','year',    "gni","water_sm",
        "I_Africa","I_Amr","I_Sear","I_Wpr","I_Emr",
        "Iage_lower_lt24m", "Iage_upper_24to59m")
names(inp.int)
dim(inp.int)
length(unique(inp.int$refid))
length(unique(inp.int$bibliography))
tapply(inp.int$bibliography, inp.int$whoreg6, FUN=function(x) length(unique(x)))
...
dw<- inp[,c("sid",causes,"n", covs,"ran_id","refid", "cases_tested",
            "cases_tested_corr","iso3", causes,"age_lower","age_upper","whoreg6")]
head(dw)
...
dws <- merge(dw[,c("sid","refid",covs,"ran_id","N","iso3","cases_tested_corr","cases_tested",
                   causes,"age_lower","age_upper","whoreg6")] , aggregate(dc[,c("g","last")], list(sid=dc$sid), max),
             by="sid")
...
#cases[order(cases$n),]
studies$year_gt2010<- ifelse(studies$year >2010,1,0)
data.predict$year_gt2010<- ifelse(data.predict$year>2010,1,0)
table(data.predict$year_gt2010)
table(studies$year_gt2010)
save(data.predict, cases, studies, natrep,
     vdr, vdt, #vid,
     vxf, vxr, file="../results/Make_input_data_20231228.RData")
